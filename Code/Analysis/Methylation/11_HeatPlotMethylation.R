...
# Input:
# BetaMatrix: A matrix of beta values for probes x patients, with probes in rows and patients as 
#             columns.
# ClinicalFile: File of patients and corresponding clinical category; matrix of size patients x 
#              clinical categories.
# CategoryToVisualize: Category to visualize from the Clinical File; Options: "SAMPLE_ID", 
#                      "SAMPLE_ID_TGL", "INCLUDE", "TIME_POINT", "RES_ID","LY_FL_ID", "OTHER_ID",
#                      "SEX","SITE_BIOPSY", "SITE_EXTRANODAL", "TYPE_BIOPSY", "TRANSLOC_14_18", 
...
HeatPlot11 <- function(BetaMatrix, 
  ...
  HeatPlot <- ggplot2::ggplot(BetaMatrix_ProbeMeans, aes(x = !! x.var, y= !! y.var)) +
  ...
  ggplot2::ggsave(paste0(pathNow, "/img/11_HeatPlot_", CategoryToVisualize, ".",PNGorPDF))
  ...
  if(length(NamesWithinCategoryToVisualize) == 3) {
    ...
    # Plot
    HeatPlot2 <- ggplot2::ggplot(BetaMatrix_ProbeMeans, aes(x = !! x.var, y = !! z.var)) +
                        stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE) +
                        scale_fill_distiller(palette = "Spectral", direction = 1) +
                        scale_x_continuous(expand = c(0, 0)) +
                        scale_y_continuous(expand = c(0, 0)) +
                        ...
                        geom_abline(slope = 1, intercept = 0) +
                        theme(aspect.ratio = 1)
    ggplot2::ggsave(paste0(pathNow, "/img/11_HeatPlot_", CategoryToVisualize,"_2.", PNGorPDF))
    ...
    HeatPlot3 <- ggplot(BetaMatrix_ProbeMeans, aes(x = !! y.var, y = !! z.var)) +
                        ...
                        geom_abline(slope = 1, intercept = 0) +
                        theme(aspect.ratio = 1)
    ggplot2::ggsave(paste0(pathNow, "/img/11_HeatPlot_", CategoryToVisualize, "_3.", PNGorPDF))
  }

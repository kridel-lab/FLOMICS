...
# TumorPurity: A numeric vector of length equalling number of samples, indicating the purity level of 
#             each sample. Only need to be provided if TypeofClustering = "InfiniumClust", "RPMM" or 
#             "all". Default is NA. 
# BetaMatrix: A matrix of beta values for probes (or genes) x patients, with probes (or genes) in rows
#             and patients as columns.
# MvalueMatrix: A matrix of M values for probes (or genes) x patients, with probes (or genes) in rows 
#               and patients as columns; or NA (default) if the MvalueMatrix is not provided.
# ListofProbes: List of probes (or genes) to serve as clustering input, that are also present in rownames 
#               of BetaMatrix and MvalueMatrix.
...
Clustering <- function(TypeofClustering = "all", 
  ...
  RNAseqHeatmap <- function() {
      ...
      # MatchRNAseqwith170 <- match(RNAseqQC18June2020T1Samples81$QCMatrixMatchedSampleFiltered$SAMPLE_ID, colnames(RNASeqCountMatrixMatchedMethylation))

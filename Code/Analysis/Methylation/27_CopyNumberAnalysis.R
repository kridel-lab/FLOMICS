...
CopyNumberAnalysis <- function(CopyNumberData, 
                               ClusterLabels) {
  
  # Match copy number data with cluster order
  Match_Copynumber_ByCluster <- match(rownames(ClusterLabels), colnames(CopyNumberData))
  CopyNumberData_Ordered <- CopyNumberData[, Match_Copynumber_ByCluster[- which(is.na(Match_Copynumber_ByCluster) == TRUE)]]
  Match_Cluster_ByCopyNumber <- match(colnames(CopyNumberData), rownames(ClusterLabels))
  ClusterLabels_Ordered <- ClusterLabels[Match_Cluster_ByCopyNumber[- which(is.na(Match_Cluster_ByCopyNumber) == TRUE)], ]
  ...
  cat("\n Analyzing data for", nrow(Combined_Dataframe)," samples across", nrow(CopyNumberData_Ordered),"sites. \n")
  ...
  Chisq.test_CopyNumberData <- lapply(c(2:60), function(i) stats::chisq.test(table(Combined_Dataframe$Cluster, Combined_Dataframe[[i]])))
  Chisq.test_CopyNumberData_Sig_PAdjusted <- Chisq.test_CopyNumberData_Sig <- matrix(unlist(lapply(c(2:60), function(w) 
  ...
  Chisq.test_CopyNumberData_Sig_PAdjusted[, 2] <- stats::p.adjust(p = as.numeric(Chisq.test_CopyNumberData_Sig[, 2]), 
                                                                  method = "bonferroni", 
  ...
  Comparing_Tables <- lapply(1:length(Chisq.test_CopyNumberData_Sig_PAdjusted[, 1]), 
                             function(u) table(Combined_Dataframe$Cluster, 
                                               Combined_Dataframe[, which(colnames(Combined_Dataframe) == Chisq.test_CopyNumberData_Sig_PAdjusted[u, 1])])) 

...
# Output
# RNASeqCountMatrixMatchedSampleFilteredNOFeatureFiltered: A matrix of raw counts, size transcripts 
#                                                          (ENSEMBL IDs) x samples
...
#                                                   A matrix of raw counts, size transcripts (ENSEMBL IDs) x 
#                                                   samples for RNAseq data, where transcripts have been filtered
...
#                                                        If RNAseqAnnotationFile is provided, then 
#                                                        RNASeqCountMatrixMatchedSampleFilteredFeatureFiltered
#                                                        with ENSEMBL IDs corresponding to only autosomes 
#                                                        (i.e., no sex or mitochondrial chromosomes) will 
#                                                        be provided. 
# RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNormalized: 
#                                                        A matrix of normalized values, size transcripts 
...
#                                                 will be provided. 
# QCMatrixMatchedSampleFiltered: QCMatrix provided as an argument, updated according 
#                                to selected sample size. Final
#                                output will be a matrix of samples x QC metrics. 
# BetaMatrixMatchedSampleFiltered: BetaMatrix provided as an argument, updated 
#                                  according to selected sample size. Final
#                                  output will be a matrix of probes x samples. 
# MvalueMatrixMatchedSampleFiltered: MvalueMatrix provided as an argument, updated 
#                                    according to selected sample size. Final
#                                    output will be a matrix of probes x samples. 
# ClinicalFileMatchedSampleFiltered: ClinicalFile provided as an argument, updated
#                                    according to selected sample size. Final
#                                    output will be a matrix of samples x clinical categories. 
# SurvivalFileMatchedSampleFiltered: SurvivalFile provided as an argument, updated 
#                                    according to selected sample size. Final
#                                    output will be a matrix of samples x survival categories. 
# TumorPurityMatchedSampleFiltered: TumorPurity provided as an argument, updated according
#                                   to selected sample size. Final
#                                   output will be a matrix of samples x survival categories.                               
# RNAseqAnnotationFileFeatureFilteredNoSexENSEMBLIDs: 
#                               RNAseqAnnotationFile provided as an argument, updated only autosomes. 
#                               Correspond to ENSEMBL IDs in 
#                               RNASeqCountMatrixMatchedSampleFilteredNOFeatureFilteredNoSexENSEMBLIDs 
#                               and RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNoSexENSEMBLIDsNormalized.
                                                     

QCRNAseq35 <- function(RNAseqCountMatrix, 
                     QCMatrix, 
  ...
  cat("\n", length(which(QCMatrixMatched$rRNAcontam > RNAseqSampleCutoffRRNAcontam)) ,
  ...
  SamplesRemove <- unique(c(QCMatrixMatched$SAMPLE_ID[! (QCMatrixMatched$Uniquely.mapped > 
                          ...
                          QCMatrixMatched$SAMPLE_ID[! (QCMatrixMatched$X..of.reads.mapped.to.multiple.loci < 
                                                         RNAseqSampleCufoffReadsMappedMultipleLoci)],
                          QCMatrixMatched$SAMPLE_ID[QCMatrixMatched$rRNAcontam > 
                                                      RNAseqSampleCutoffRRNAcontam]))
  ...
  cat("\n", length(SamplesRemove), "samples will be removed all toegther. \n
      New total number of samples remaining is ", 
  ...
  if(length(SamplesRemove) > 0) { # if samples to remove are present
    ...
    if(class(BetaMatrix)[1] != "logical") {
      BetaMatrixMatchedSampleFiltered <- BetaMatrix[, 
                                                    match(colnames(RNASeqCountMatrixMatchedSampleFiltered), 
                                                    colnames(BetaMatrix))]
    }
    # dim(BetaMatrixMatchedSampleFiltered) # 595564     58
    
    if(class(MvalueMatrix)[1] != "logical") {
      MvalueMatrixMatchedSampleFiltered <- MvalueMatrix[, 
                                                        match(colnames(RNASeqCountMatrixMatchedSampleFiltered), 
    ...
    if(class(TumorPurity)[1] != "logical") {
      TumorPurityMatchedSampleFiltered <- as.matrix(TumorPurity[match(colnames(RNASeqCountMatrixMatchedSampleFiltered), 
                                                                      rownames(TumorPurity)), ], ncol = 1)
      rownames(TumorPurityMatchedSampleFiltered) <- rownames(TumorPurity)[match(colnames(RNASeqCountMatrixMatchedSampleFiltered), 
                                                                                rownames(TumorPurity))]
    }
    ...
    if(class(TumorPurity)[1] != "logical") {
      TumorPurityMatchedSampleFiltered <- 
        as.matrix(TumorPurity[match(colnames(RNASeqCountMatrixMatchedSampleFiltered), 
        rownames(TumorPurity)), ], ncol = 1)
      rownames(TumorPurityMatchedSampleFiltered) <- 
        rownames(TumorPurity)[match(colnames(RNASeqCountMatrixMatchedSampleFiltered), 
        rownames(TumorPurity))]
    }
  ...
  if (length(which(colSums(RNASeqCountMatrixMatchedSampleFiltered) == 0)) > 0) {  
    ...
    RNASeqCountMatrixMatchedSampleFiltered <- RNASeqCountMatrixMatchedSampleFiltered[, - 
                                              which(colSums(RNASeqCountMatrixMatchedSampleFiltered) == 0)]
  ...
  if(length(which(rowSums(RNASeqCountMatrixMatchedSampleFiltered) == 0)) > 0) {
    RNASeqCountMatrixMatchedSampleFiltered <- RNASeqCountMatrixMatchedSampleFiltered[-
                                              which(rowSums(RNASeqCountMatrixMatchedSampleFiltered) == 0), ]
    # dim(RNASeqCountMatrixMatchedSampleFiltered) # 50911    58
    
    # checking Huet et al genes
    # matchZeroFeatrues <- match(names(which(rowSums(RNASeqCountMatrixMatchedSampleFiltered) == 0)), ENSMBLid$gene)
    # matchNonZeroFeatrues <- match(rownames(RNASeqCountMatrixMatchedSampleFiltered), ENSMBLid$gene)
    # matchedEntries <- match(c(goodGenes, badGenes), ENSMBLid$name[matchNonZeroFeatrues])
    # ENSMBLid$name[matchNonZeroFeatrues][matchedEntries]
  ...
  if(RNAseqFeatureSelectionMethod == "SD") { # using standard deviation
    ...
    cat("\n", nrow(RNASeqCountMatrixMatchedSampleFiltered) - nrow(SDProbeFilterRNAseq$BetaMatrix_SD_Filtered), 
        "probes will be removed for RNAseq. New probe size is", 
        nrow(SDProbeFilterRNAseq$BetaMatrix_SD_Filtered), ".")
    ...
    cat("\n", nrow(RNASeqCountMatrixMatchedSampleFiltered) - nrow(MADProbeFilterRNAseq$BetaMatrix_MAD_Filtered), 
        "probes will be removed for RNAseq. New probe size is", 
        nrow(MADProbeFilterRNAseq$BetaMatrix_MAD_Filtered), ".")
    ...
    # "ENSG00000234965" - "SHISA8"
    # "ENSG00000169575" - "VPREB1"
    # For Huet et al study 
    # keepTranscripts[which(names(keepTranscripts) == c("ENSG00000234965", "ENSG00000169575"))] <- TRUE
    ...
    # checking Huet et al genes
    # matchFeatrues <- match(rownames(exprsRNAseq), ENSMBLid$gene)
    #  matchGenes <- match(c(goodGenes, badGenes), ENSMBLid$name[matchFeatrues])
    # ENSMBLid$gene[matchFeatrues][matchGenes]
  }
  ...
  if(RNAseqNormalizationMethod == "StandardNormalize" || is.na(RNAseqNormalizationMethod)) {
    ...
    Data2RNAseqFilterededgeRNorm <- t(SNFtool::standardNormalization(log2(1 + 
                                      Data2RNAseq_FilteredSamplesProbes)))
    # dim(Data2RNAseqFilterededgeRNorm) # 34749    58
      ...
      if (RNAseqFeatureSelectionMethod == "edgeR") {
        ...
        # Generate matrix
        Data2RNAseqFilterededgeRNormLOG <- edgeR::cpm(Data2RNAseqFilterededgeRNormFactors, 
  ...
  if (all(is.na(RNAseqAnnotationFile)) == FALSE) {
    ...
    Data2RNAseqFilterededgeRNormNoSex <- Data2RNAseqFilterededgeRNorm[match(rownames(chrOfIDsTableAutosomesOnly), rownames(Data2RNAseqFilterededgeRNorm)), ]
    Data2RNAseqFilterededgeRNormLOGNoSex <- Data2RNAseqFilterededgeRNormLOG[match(rownames(chrOfIDsTableAutosomesOnly), rownames(Data2RNAseqFilterededgeRNormLOG)), ]
    
    Data2RNAseqFilterededgeRNormNoSexAnnotations <- chrOfIDsTable %>%
      tibble::rownames_to_column('gene') %>%
      dplyr::filter(gene %in% rownames(Data2RNAseqFilterededgeRNormNoSex)) %>%
      data.frame() %>%
      dplyr::select(gene, chromosome, name, type) %>%
      dplyr::bind_cols(data.frame(Data2RNAseqFilterededgeRNormNoSex))
    
    
    # Saving results - if RNAseq annotation file is provided
    RESULTS <- list(RemovedSamples = SamplesRemove,
                    RNASeqCountMatrixMatchedSampleFilteredNOFeatureFiltered = 
                      RNASeqCountMatrixMatchedSampleFilteredNOFeatureFiltered,
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFiltered = 
                      RNASeqCountMatrixMatchedSampleFilteredFeatureFiltered,
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNoSexENSEMBLIDs = 
                      chrOfIDsTableAutosomesOnly[, c(4:ncol(chrOfIDsTableAutosomesOnly))],
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNoSexENSEMBLIDsWithAnnotations = 
                      chrOfIDsTableAutosomesOnly, 
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNormalized = 
                      Data2RNAseqFilterededgeRNorm,
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNoSexENSEMBLIDsNormalized = 
                      Data2RNAseqFilterededgeRNormNoSex,
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNoSexENSEMBLIDsNormalizedLOG = 
                      Data2RNAseqFilterededgeRNormLOGNoSex,
                    RNASeqCountMatrixMatchedSampleFilteredFeatureFilteredNoSexENSEMBLIDsNormalizedWithAnnotations = 
                      Data2RNAseqFilterededgeRNormNoSexAnnotations, 
                    QCMatrixMatchedSampleFiltered = QCMatrixMatchedSampleFiltered,
                    BetaMatrixMatchedSampleFiltered = BetaMatrixMatchedSampleFiltered,
                    MvalueMatrixMatchedSampleFiltered = MvalueMatrixMatchedSampleFiltered,
                    ClinicalFileMatchedSampleFiltered = ClinicalFileMatchedSampleFiltered,

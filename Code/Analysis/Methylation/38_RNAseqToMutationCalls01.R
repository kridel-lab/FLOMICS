...
#                   If ClinicalCategory is provided then association with mutation status will be 
#                   performed using  chi-squared test and Fisher's exact statistic. The "ID"s will 
#                   be matched to be same length as number of SAMPLE IDs in RNAseqMutationCalls. 
...
RNAseqToMutationCalls01 <- function(RNAseqMutationCalls, 
  ...
  if(! all(is.na(VariantClassificationToRemove))) {
    ...
    rownames(RNAseqMutationCallsFiltered01) <- rownames(FrequencyofRNAseqMutations) <- 
      unique(RNAseqMutationCallsFiltered$SAMPLE_ID)
    colnames(RNAseqMutationCallsFiltered01) <- colnames(FrequencyofRNAseqMutations) <- 
      unique(RNAseqMutationCallsFiltered$Hugo_Symbol)
    
    
    # In the 0 matrix, only write 1 if the muation is present 
    # for each sample
    cat("\n A total of ", length(unique(RNAseqMutationCallsFiltered$SAMPLE_ID)), 
        "samples present to analyze. \n")
    ...
    for (i in 1:length(unique(RNAseqMutationCallsFiltered$SAMPLE_ID))) {
      ...
      for (j in 1:nrow(filter(RNAseqMutationCallsFiltered, SAMPLE_ID == 
                              unique(RNAseqMutationCallsFiltered$SAMPLE_ID)[i]))) {
        ...
        colName <- match(filter(RNAseqMutationCallsFiltered, SAMPLE_ID == 
                                  unique(RNAseqMutationCallsFiltered$SAMPLE_ID)[i])$Hugo_Symbol[j], 
    ...
    if(! all(is.na(ClusterLabels))) {
      ...
      for (k in 1:ncol(RNAseqMutationCallsFiltered01ClusterLabs)) { # for each gene/mutation
       #  cat("\nk is:", k)
        if(! all(RNAseqMutationCallsFiltered01ClusterLabs[, k] == 0, na.rm = T)) { # only if all entries are not zero
          ...
          RNAseqMutationCalls01ClusterLabsSignficance[k, 9] <- 
            length(which(RNAseqMutationCallsFiltered01ClusterLabs[, k] == 1))
          RNAseqMutationCalls01ClusterLabsSignficance[k, 10] <- 
            (length(which(RNAseqMutationCallsFiltered01ClusterLabs[, k] == 1)) / 
            length(RNAseqMutationCallsFiltered01ClusterLabs[, k]))
          RNAseqMutationCalls01ClusterLabsSignficance[k, 11] <- 
            (sum(FrequencyofRNAseqMutationsClusterLabs[, k]))
        }
      }
      ...
      AssociationMutationClustLabsSigChiSquared <- 
        RNAseqMutationCalls01ClusterLabsSignficance[which(RNAseqMutationCalls01ClusterLabsSignficance[, 2] < 0.05), ]
      AssociationMutationClustLabsSigFisher <- 
        RNAseqMutationCalls01ClusterLabsSignficance[which(RNAseqMutationCalls01ClusterLabsSignficance[, 8] < 0.05), ]
      AssociationMutationClustLabsSigChiSquaredAndFisher <- 
        RNAseqMutationCalls01ClusterLabsSignficance[
      ...
      AssociationMutationClustLabsPMpanel <- 
        RNAseqMutationCalls01ClusterLabsSignficance[matchPMwithRNAseqMutCalls[! is.na(matchPMwithRNAseqMutCalls)], ]
      ...
      if (ProduceImages == "Yes") {
        ...
        PlotPMPanel <- ggplot2::ggplot(data = RNAseqMutationCalls01SignficancePMPanelFilteredPlottingT1, 
                       ...
                       ggrepel::geom_label_repel(aes(label = 
                                                 rownames(RNAseqMutationCalls01SignficancePMPanelFilteredPlottingT1)),
                                                 box.padding   = 0.35, 
                                                 point.padding = 0.5,
      ...
      RESULTS <- list(MutationCalls01 = data.frame(RNAseqMutationCallsFiltered01),
                     AssociationMutationClustLabs = data.frame(RNAseqMutationCalls01ClusterLabsSignficance),
                     AssociationMutationClustLabsSigChiSquared = data.frame(AssociationMutationClustLabsSigChiSquared),
                     AssociationMutationClustLabsSigFisher = data.frame(AssociationMutationClustLabsSigFisher),
                     AssociationMutationClustLabsSigChiSquaredAndFisher = 
                       data.frame(AssociationMutationClustLabsSigChiSquaredAndFisher),
                     AssociationMutationClustLabsPMpanel = data.frame(AssociationMutationClustLabsPMpanel))
      ...
      FrequencyofRNAseqMutationsClinicalCategory <- data.frame(cbind(as.character(ClinicalCategoryMatched), 
                                                                FrequencyofRNAseqMutations))
      FrequencyofRNAseqMutationsClinicalCategory <- FrequencyofRNAseqMutationsClinicalCategory[- 
                                                    which(is.na(FrequencyofRNAseqMutationsClinicalCategory$V1) == TRUE), ]
      
      RNAseqMutationCallsFiltered01ClinicalCategory <- data.frame(cbind(as.character(ClinicalCategoryMatched),
                                                                   RNAseqMutationCallsFiltered01))
      RNAseqMutationCallsFiltered01ClinicalCategory <- RNAseqMutationCallsFiltered01ClinicalCategory[- 
                                                       which(is.na(RNAseqMutationCallsFiltered01ClinicalCategory$V1) == TRUE), ]
      ...
      colnames(RNAseqMutationCalls01ClinicalSignficance) <- c("Xsquared", "XsquaredP.Value", 
                                                                 paste0("XsquaredRes1-", 
                                                                        levels(RNAseqMutationCallsFiltered01ClinicalCategory$ClinicalCategory)[1]), 
                                                                 paste0("XsquaredRes0-", 
                                                                        levels(RNAseqMutationCallsFiltered01ClinicalCategory$ClinicalCategory)[1]),
                                                                 paste0("XsquaredRes1-", 
                                                                        levels(RNAseqMutationCallsFiltered01ClinicalCategory$ClinicalCategory)[2]),
                                                                 paste0("XsquaredRes0-", 
                                                                        levels(RNAseqMutationCallsFiltered01ClinicalCategory$ClinicalCategory)[2]),
                                                                 "fisherTOddsRatio", 
                                                                 "fisherTP.Value",
      ...
      # save results that are significant with respect to chi-squared or Fisher's Exact Test or both tests
      AssociationMutationClinicalSigChiSquared <- 
        RNAseqMutationCalls01ClinicalSignficance[which(RNAseqMutationCalls01ClinicalSignficance[, 2] < 0.05), ]
      AssociationMutationClinicalSigFisher <- 
        RNAseqMutationCalls01ClinicalSignficance[which(RNAseqMutationCalls01ClinicalSignficance[, 8] < 0.05), ]
      AssociationMutationClinicalSigChiSquaredAndFisher <- 
        RNAseqMutationCalls01ClinicalSignficance[
      ...
      RESULTS <- list(MutationCalls01 = data.frame(RNAseqMutationCallsFiltered01),
                      AssociationMutationClinicalCategory = 
                        data.frame(RNAseqMutationCalls01ClinicalSignficance),
                      AssociationMutationClinicalCategorySigChiSquared = 
                        data.frame(AssociationMutationClinicalSigChiSquared),
                      AssociationMutationClinicalCategorySigFisher = 
                        data.frame(AssociationMutationClinicalSigFisher),
                      AssociationMutationClinicalCategorySigChiSquaredAndFisher = 
                        data.frame(AssociationMutationClinicalSigChiSquaredAndFisher),
                      AssociationMutationClinicalCategoryPMpanel = 
                        data.frame(AssociationMutationClinicalPMpanel))

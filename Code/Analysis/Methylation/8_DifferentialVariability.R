...
# Function: Differential variability with no summarization 
# Rather than testing for differences in mean methylation, test for differences between group variances.
...
# ContrastColumnName is type of differential analysis to be done. ContrastColumnName options include "STAGE", "SEX", "SITE_BIOPSY", "TYPE_BIOPSY", "COO", "TRANSLOC_14_18", BATCH.
...
DifferentialVariability <- function(ClinicalFile, 
  ...
  #LoadCheckPkg(RegularPckgs=c("limma","missMethyl","IlluminaHumanMethylationEPICanno.ilm10b2.hg19","ggplot2","ggrepel","stringi"))
  library(limma)
  library(missMethyl)
  ...
  if(ContrastColumnName == "STAGE") {
    ...
    fitvar <- varFit(data = MvalueMatrix[,which(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")] == "FL")], 
    ...
    design_epic2 <- model.matrix (~ 0 + stage_status)
    colnames(design_epic2) <- levels(stage_status)
    fitvar <- varFit(data = MvalueMatrix[ , - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "SITE_BIOPSY")] == TRUE)))],
                     design = design_epic2, coef = c(1, 2))
    summary <- summary(decideTests(fitvar))
    topDV <- topVar(fitvar, coef = 2)
    ...
    design_epic2 <- model.matrix (~ 0 + stage_status)
    colnames(design_epic2) <- levels(stage_status)
    fitvar <- varFit(data = MvalueMatrix[ , - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE_BIOPSY")]) == "TRUE"))], 
                     design = design_epic2, coef = c(1, 2))
    summary <- summary(decideTests(fitvar))
    topDV <- topVar(fitvar, coef = 2)
    ...
    design_epic2 <- model.matrix (~ 0 + stage_status)
    colnames(design_epic2) <- levels(stage_status)
    fitvar <- varFit(data = MvalueMatrix[ , which(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")] == "DLBCL")], 
                     design = design_epic2, coef = c(1, 2))
    summary <- summary(decideTests(fitvar))
    topDV <- topVar(fitvar, coef = 2)
    ...
    design_epic2 <- model.matrix (~ 0 + stage_status)
    colnames(design_epic2) <- levels(stage_status)
    fitvar <- varFit(data = MvalueMatrix[ , which(! is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE)], 
                     design = design_epic2, coef = c(1, 2))
    summary <- summary(decideTests(fitvar))
    topDV <- topVar(fitvar, coef = 2)

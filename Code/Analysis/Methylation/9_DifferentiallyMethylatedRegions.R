...
# Updated 3 June 2019
# Function: bumphunter package looks for genomic regions that are differentially methylated between two conditions
# https://htmlpreview.github.io/?https://github.com/hansenlab/tutorial.450k/blob/master/inst/doc/methylation450k.html#bumphunter-to-find-differentially-methylated-regions-dmrs

# DMRcate extracts the most differentially methylated regions (DMRs) and variably methylated regions (VMRs) from IlluminaR Infinium BeadChip Array samples via kernel smoothing.
# http://bioconductor.org/packages/release/bioc/vignettes/DMRcate/inst/doc/DMRcate.pdf
...
# ContrastColumnName: Type of differential analysis to be done. ContrastColumnName options include "STAGE", "SEX", "SITE_BIOPSY", "TYPE_BIOPSY", "INSTITUTION", "COO", "TYPE", "TRANSLOC_14_18", "EPIC_QC", "CLUSTER".
...
# ExpressionFile: A string indicating the name of expression file in text format, if present. Otherwise, set to NA. 
...
# DMRcate_output
# AnnotatedCpGs: A list length of the number of contrasts in ContrastColumnName, that contains annotated CpGs with their chromosome position and test statistic.
# DifferentiallyMethylatedRegions: A list length of the number of contrasts in ContrastColumnName, that contains significantly differentially (or variable) methylated regions.
# GRangesObject: A list length of the number of contrasts in ContrastColumnName, that contains corresponding GRanges object. 
...
DiffMethylatedRegions <- function(Method = "DMRcate", 
  ...
  # LoadCheckPkg(RegularPckgs=c("stringi","bumphunter"))
  library(stringi)
  library(limma)
  ...
  if(ContrastColumnName == "STAGE"){
    stage_status <- factor(stri_trim(ClinicalFile[which(ClinicalFile[, which(colnames(ClinicalFile) =="TYPE")]=="FL"), which(colnames(ClinicalFile) == "STAGE")]), levels = c("ADVANCED", "LIMITED"))
    design_epic2 <- stats::model.matrix (~stage_status)
  ...
  DMRcate_function <- function(BetaMatrix, 
    ...
    if(ContrastColumnName == "STAGE") {
      ...
      if(length(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "STAGE")]) == TRUE)) > 0) {
        stage_status <- factor(stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "STAGE")]) == TRUE)), 
        ...
        stage_status <- factor(stri_trim(ClinicalFile[,which(colnames(ClinicalFile) == "STAGE")]), levels = c("ADVANCED", "LIMITED"))
      }
      ...
      if(length(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "SEX")]) == TRUE)) > 0) {
        stage_status <- factor(stri_trim(ClinicalFile[-c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "SEX")]) == TRUE)), 
                                                      which(colnames(ClinicalFile) == "SEX")]),  levels = c("M", "F"), labels = c("Male", "Female"))
        MvalueMatrix <- MvalueMatrix[, which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == ContrastColumnName)]) == FALSE)]
      } else {
        stage_status <- factor(stri_trim(ClinicalFile[, which(colnames(ClinicalFile) == "SEX")]),  levels = c("M", "F"), labels = c("Male","Female"))
      }
      design_epic2 <- stats::model.matrix(~0 + stage_status)
      colnames(design_epic2) <- levels(stage_status)
      ...
      if(length(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "INSTITUTION")]) == TRUE)) > 0) {
        stage_status <- factor(stri_trim(ClinicalFile[-c(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "INSTITUTION")]) == TRUE)), 
      ...
      if(length(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "EPIC_QC")]) == TRUE)) > 0) {
        stage_status <- factor(stri_trim(ClinicalFile[-c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "EPIC_QC")]) == TRUE)), 
      ...
      if(length(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "COO")]) == TRUE)) > 0) {
        stage_status <- factor(stri_trim(ClinicalFile[-c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "COO")]) == TRUE)), 
      ...
      if(length(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")]) == TRUE)) > 0) {
        if (length(unique(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")])) == 2) {
          stage_status <- factor(stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")]) == TRUE)), 
        ...
        if (length(unique(ClinicalFile[, which(colnames(ClinicalFile) == "TYPE")])) == 2) {
          stage_status <- factor(stri_trim(ClinicalFile[, which(colnames(ClinicalFile) == "TYPE")]))
        } else {
            stage_status <- factor(stri_trim(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")]), 
      ...
      if (length(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE)) > 0) {
        stage_status <- factor(stri_trim(ClinicalFile[-c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE)),

...
# 18 February 2021
# Developed by Anjali Silva and Robert Kridel

# Function copies from: https://github.com/PeeperLab/Rtoolbox/blob/master/R/ReplotGSEA.R
...
## enrichment.score.range: the range of the enrichment score; defaults to 
## [min(ENRICHMENT SCORE), max(ENRICHMENT SCORE)]
...
replotGSEA <- function(path, gene.set, class.name, metric.range,
  ...
  pdf("img/GSEA.pdf", width = 2.5, height = 2)
  
  gsea.layout <- layout(matrix(c(1, 2, 3)),
                        heights = c(1.7, 0.5, 0.2, 0.2))
  ...
  if(gsea.enrichment.score < 0) {
    ...
    text(length(gsea.rnk$metric) * 0.99, plot.coordinates[4] - 
           ((plot.coordinates[4] - plot.coordinates[3]) * 0.03),
         paste("Nominal p-value:", gsea.p.value, "\nFDR:", gsea.fdr, "\nES:",
               gsea.enrichment.score, "\nNormalized ES:",
               gsea.normalized.enrichment.score, "\n"), adj = c(1, 1))
  }
  ...
  barplot(matrix(rank.colors$lengths), col = rank.colors$values, 
          border = NA, horiz = TRUE, xaxt = "n", xlim = c(1, length(gsea.rnk$metric)))
  box()
...
replotGSEA("RNAseq/GSEA/GSEA_results_bySNFClust/gsea",
           "PROLIF-10", "")
dev.off()

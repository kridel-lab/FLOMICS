# Downloaded from FLOMICS GitHub 22 March 2021
# Edited by Anjali on 22 March 2021


###
# This script is based on original script from Arash (Pugh Lab, Princess Margaret Cancer Centre).
# It reads in output from MiXCR
# and then runs iNext to calculate diversity indices.
# https://cran.r-project.org/web/packages/iNEXT/vignettes/Introduction.html
###

setwd("~/UHN/kridel-lab - FLOMICS")
...
lst <- immunelistfx(datapath = "~/UHN/kridel-lab - FLOMICS/Analysis-Files/Mixcr/specific_clonotypes/", 
                    chain = "IGH")
...
Divstats.fx <- function(lst, chain, batchname, outpath) {
  ...
  # Estimators
  out <- iNEXT::iNEXT(lst, 0, datatype="abundance")
...
Divstats.fx(lst, chain = "TRB", batchname = "", outpath = "/Users/anjalisilva/Desktop/")


out <- iNEXT::iNEXT(lst, 0, datatype = "abundance")

iNEXT::ggiNEXT(out, type=2, facet.var = "none", color.var = "site") +
              ylim(0,1) +
              theme_bw() +
...
lstIGH <- immunelistfx(datapath = "/Users/as/UHN/kridel-lab - FLOMICS/Analysis-Files/Mixcr/specific_clonotypes/", 
                       chain = "IGH")

Divstats.fx(lst = lstIGH, 
            chain = "IGH", 
            batchname = "", 
            outpath = "/Users/as/UHN/kridel-lab - FLOMICS/Analysis-Files/Mixcr/Arash_script_output/")

outlstIGH <- iNEXT::iNEXT(x = lstIGH, q = 0, datatype = "abundance")

iNEXT::ggiNEXT(outlstIGH, type = 2, facet.var = "none", color.var="site") +
                ylim(0,1) +
                theme_bw() +
                theme(legend.position = "none")

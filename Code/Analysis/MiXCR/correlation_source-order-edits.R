...
pdf(paste("/Users/sarahrussell/R/plots/", date, "BISQUE_correlations.pdf", sep="_"), width=10,height=6)
#make plot
c1=cors(data1) %>% filter(., grepl(paste(celltype, collapse="|"), measure1) & !grepl(paste(celltype, collapse="|"), measure2))
c1$measure1=  gsub("\\."," ", c1$measure1)
c1$measure2=  gsub("_"," ", c1$measure2)

g1=ggplot(data=c1, mapping=aes(measure1, measure2, fill=r, label=round(r_if_sig,2))) +
      geom_tile() +
...
c2=cors(data1) %>% filter(., grepl(paste(celltype, collapse="|"), measure1) & !grepl(paste(celltype, collapse="|"), measure2))
c2$measure1=  gsub("\\."," ", c2$measure1)
c2$measure2=  gsub("_"," ", c2$measure2)
g2=ggplot(data=c2, mapping=aes(measure1, measure2, fill=r, label=round(r_if_sig,2))) +
      geom_tile() +
      labs(x = NULL, y = NULL, fill = "Spearman's\nCorrelation",
      subtitle="Only significant Spearman's correlation coefficients shown") + scale_fill_gradient2(mid="#FBFEF9",low="#0C6291",high="#A63446", limits=c(-1,1)) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
make_plot <- function(df,gr){
  c=df %>% filter(., grepl(paste(celltype, collapse="|"), measure1) & !grepl(paste(celltype, collapse="|"), measure2))
  c$measure1=gsub("\\."," ", c$measure1)
  c$measure2=gsub("_"," ", c$measure2)

  g=
    ggplot(data=c, mapping=aes(measure1, measure2, fill=r, label=round(r_if_sig,2))) +
    geom_tile() +
...
pdf(paste("/Users/sarahrussell/R/plots/", date, comparison, "correlations_groups.pdf", sep="_"), width=10,height=6)

...
DifferentialExpressionRNAseq <- function(RNAseqCountMatrix, 
  ...
  if(ContrastColumnName == "STAGE") {
    ...
    statusContrastName <- factor(stringi::stri_trim( ClinicalFile[which(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")] == "FL"), 
                                     which(colnames(ClinicalFile) == "STAGE")]), levels = c("ADVANCED", "LIMITED"))
    exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, which(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")] == "FL")]
    ...
    if(length(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "SEX")]) == TRUE)) > 0) {
      statusContrastName <- factor(stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "SEX")]) == TRUE)), 
                                                          which(colnames(ClinicalFile) == "SEX")]),  
                                   levels = c("M", "F"), labels = c("Male", "Female"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "SEX")]) == TRUE))]
      
    } else {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[ , which(colnames(ClinicalFile) == "SEX")]),  levels = c("M", "F"), 
    ...
    if(length(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "SITE_BIOPSY")]) == TRUE)) > 0) {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "SITE_BIOPSY")]) == TRUE)), 
                                                          which(colnames(ClinicalFile) == "SITE_BIOPSY")]),  levels = c("EN", "LN"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "SITE_BIOPSY")]) == TRUE))]
    }else {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "SITE_BIOPSY")] == TRUE))), 
                                                          which(colnames(ClinicalFile) == "SITE_BIOPSY")]), levels = c("EN", "LN"))
    }
    ...
    if(length(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "TYPE_BIOPSY")]) == TRUE)) > 0) {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "TYPE_BIOPSY")]) == TRUE)), 
                                                          which(colnames(ClinicalFile) == "TYPE_BIOPSY")]), levels = c("CORE", "TISSUE"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE_BIOPSY")]) == TRUE))]
    ...
    if(length(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "INSTITUTION")]) == TRUE)) > 0) {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "INSTITUTION")]) == TRUE)), 
                                                          which(colnames(ClinicalFile) == "INSTITUTION")]), levels = c("BCCA", "UHN","JGH"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "INSTITUTION")]) == TRUE))]
    } else {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[ , which(colnames(ClinicalFile) == "INSTITUTION")]), levels = c("BCCA", "UHN","JGH"))
    }
    designRNAseq <- stats::model.matrix(~0 + statusContrastName)
    colnames(designRNAseq) <- levels(statusContrastName)
    ...
    if(length(which(is.na(ClinicalFile[,which(colnames(ClinicalFile) == "EPIC_QC")]) == TRUE)) > 0) {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "EPIC_QC")]) == TRUE)), 
                                                          which(colnames(ClinicalFile) == "EPIC_QC")]), levels = c("Fair", "Good", "Poor"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "EPIC_QC")]) == TRUE))]
      
    } else {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[ , which(colnames(ClinicalFile) == "EPIC_QC")]), levels = c("Fair", "Good", "Poor"))
    }
    designRNAseq <- stats::model.matrix(~0 + statusContrastName)
    colnames(designRNAseq) <- levels(statusContrastName)
    ...
    if(length(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "COO")]) == TRUE)) > 0) {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "COO")]) == TRUE)), 
                                                          which(colnames(ClinicalFile) == "COO")]), levels = c("GCB", "ABC"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "COO")]) == TRUE))]
     } else {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[which(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")] == "DLBCL"), 
                                                          which(colnames(ClinicalFile) == "COO")]), levels = c("GCB", "ABC"))
    }
    designRNAseq <- stats::model.matrix(~0 + statusContrastName)
    colnames(designRNAseq) <- levels(statusContrastName)
    ...
    if(length(which(is.na(ClinicalFile[, which(colnames(ClinicalFile) == "TYPE")]) == TRUE)) > 0) {
      ...
      if (length(unique(ClinicalFile[,which(colnames(ClinicalFile) == "TYPE")])) == 2) {
        ...
        statusContrastName <- factor(stringi::stri_trim(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")]), levels = c("DLBCL", "FL", "RLN"))
      }
    }
    designRNAseq <- stats::model.matrix(~0 + statusContrastName)
    colnames(designRNAseq) <- levels(statusContrastName)
    if (length(unique(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")])) == 2) {
      contrasts <- limma::makeContrasts(paste0(unique(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")])[1], "-", 
                                               unique(ClinicalFile[ , which(colnames(ClinicalFile) == "TYPE")])[2]), levels = designRNAseq)
    } else {
    ...
    if (length(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE)) > 0) {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[- c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE)),
                                                          which(colnames(ClinicalFile) == "TRANSLOC_14_18")]),  
                                   levels = c("0", "1"), labels = c("NoTranslocation", "Translocation"))
      exprsRNAseqNormFacs <- exprsRNAseqNormFacs[, - c(which(is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE))]
    } else {
      statusContrastName <- factor(stringi::stri_trim(ClinicalFile[which(! is.na(ClinicalFile[ , which(colnames(ClinicalFile) == "TRANSLOC_14_18")]) == TRUE),
                                                          which(colnames(ClinicalFile) == "TRANSLOC_14_18")]), 
                                   levels = c("0", "1"), labels = c("NoTranslocation", "Translocation"))
    }
    designRNAseq <- stats::model.matrix(~0 + statusContrastName)
  ...
  if(ProduceHeatMap == "Yes" && (all(is.na(ClusterLabels)) == FALSE)) {
    ...
    EachContrastData2RNAseqFilterededgeRNorm [[i]] <- Data2RNAseqFilterededgeRNorm <- edgeR::cpm(Data2RNAseqFilterededgeRNormFactors, 

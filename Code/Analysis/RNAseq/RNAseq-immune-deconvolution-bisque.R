...
bisque_pc = readRDS("Analysis-Files/Seurat/bisque_decomposed_samples_pc.rds")
bisque_nc = readRDS("Analysis-Files/Seurat/bisque_decomposed_samples_nc.rds")

#----------------------------------------------------------------------
#analysis
#----------------------------------------------------------------------

get_bisque_summ = function(dat, type){
  ...
  pdf(paste("Analysis-Files/Immune-Deconvolution/", date, "_", type, "_BISQUE_results.pdf", sep=""), width=15)
  g1 = ggboxplot(filter(immune_cells, !(is.na(STAGE))), x="cell_type", y="value", fill="STAGE") +
  theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  stat_compare_means(aes(group = STAGE), label = "p.signif") + ggtitle(type)

  g2 = ggboxplot(immune_cells, x="cell_type", y="value", fill="TYPE") +
  theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  stat_compare_means(aes(group = TYPE), label = "p.signif") + ggtitle(type)

  g3 = ggboxplot(immune_cells, x="cell_type", y="value", fill="Cluster",palette = "jco") +
  theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  stat_compare_means(aes(group = Cluster), label = "p.signif") + ggtitle(type)
...
get_bisque_summ(bisque_pc, "via protein-coding genes only")
get_bisque_summ(bisque_nc, "via non-coding genes only")

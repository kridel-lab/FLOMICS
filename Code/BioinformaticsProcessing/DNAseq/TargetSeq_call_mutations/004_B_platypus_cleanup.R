...
setwd("/cluster/projects/kridelgroup/FLOMICS/ANALYSIS/PLATYPUS")
...
clean_up_001 = function(paired_vcf){
  ...
  #try 30X
  vcf = as.data.table(filter(vcf, TC >=30))
  ...
  gt$FR = as.numeric(gt$FR)
  gt = as.data.table(filter(gt, (FR < 0.001 | is.na(FR))))
...
date, "_Platypus_filtered_mutations_FL.txt", sep=""), quote=F, row.names=F, sep=";")

...
if [ ! -d  $work_path/RNAseqQC_out_path ]
then mkdir -p $work_path/RNAseqQC_out_path
fi

files_location=${work_path}/input_bams
out_path=${work_path}/RNAseqQC_out_path
#cd $out_path
...
if [[ "$input_bam" =~ ${files_location}/(.*)Aligned.sortedByCoord.out.bam ]]
 ...
 echo "$prefix"
 if [ ! -d  $work_path/RNAseqQC_out_path/${prefix} ]
  then mkdir -p $work_path/RNAseqQC_out_path/${prefix}
 fi
 qualimap rnaseq -bam ${input_bam} -gtf ${genomic_ref}/gencode.v37lift37.annotation.gtf -outdir $out_path/${prefix} --java-mem-size=41440M && echo "$prefix rnaseq done"
fi

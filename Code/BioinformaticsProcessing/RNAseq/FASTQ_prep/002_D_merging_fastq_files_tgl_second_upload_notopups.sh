...
#SBATCH --array=0-7 # job array index - number of jobs = numb of unique samples with top up runs
...
#nontopup = nontopup[-1,] #removing LY_DLC_001 because it does have a top-up
#nontopup = (as.data.frame(unique(nontopup)))
...
#pwd
samples=/cluster/projects/kridelgroup/FLOMICS/ANALYSIS/TOPUP_ANALYSIS_RNASEQ/topups_unique_sample_IDs_notopups.txt
names=($(cat $samples))
...
#pwd
#samples=/cluster/projects/kridelgroup/FLOMICS/TOPUP_ANALYSIS_RNASEQ/topups_unique_sample_IDs.txt
#names=($(cat $samples))
#echo ${names[${SLURM_ARRAY_TASK_ID}]}

...
#ANALYSIS
#----------------------------------------------------------------------------------
cdr3_BCR=as.data.table(filter(cdr3, !grepl('TR', vhits)))
dim(cdr3_BCR)
#[1] 38802       8
length(unique(cdr3_BCR$sample))
#[1] 130

#remove sequences that have frameshift or stop codon
cdr3_TCR <- cdr3_BCR[!grepl("\\~",cdr3_BCR$aaSeqCDR3),]
cdr3_TCR <- cdr3_BCR[!grepl("\\*",cdr3_BCR$aaSeqCDR3),]
cdr3_TCR <- cdr3_BCR[!grepl("\\_",cdr3_BCR$aaSeqCDR3),]


ID=as.character(unique(cdr3_BCR$sample))
get_uniq_CDR=function(ID){
    count=length(unique(cdr3_BCR[cdr3_BCR$sample==ID,]$aaSeqCDR3))
    count_matrix=as.data.frame(cbind(count,ID))
    count_matrix$type <- ifelse(grepl("DLC",count_matrix$ID)==TRUE, "DLC",
      ifelse(grepl("FL",count_matrix$ID)==TRUE,"FL","RLN"))
    return(count_matrix)
...
stage=as.character(cdr3_BCR$stage[match(unique(cdr3_BCR$sample),cdr3_BCR$sample)])
cluster=as.character(cdr3_BCR$cluster[match(unique(cdr3_BCR$sample),cdr3_BCR$sample)])
all_counts=as.data.frame(cbind(counts,stage,cluster))
...
ggboxplot(all_counts, x = "type", y = "count",
          title = "Number of unique T cell clones (unique CDR3 sequence) per type", ylab = "BCR clones", xlab = "Sample Type",
          color = "type", palette = "jco",
          add = "jitter")
dev.off()
...
ggboxplot(all_counts[all_counts$type=="FL",], x = "stage", y = "count",
          title = "Number of unique T cell clones (unique CDR3 sequence) per stage", ylab = "BCR clones", xlab = "Sample Stage",
          color = "stage", palette = "jco",
          add = "jitter")
dev.off()
...
ggboxplot(all_counts, x = "cluster", y = "count",
          title = "Number of unique T cell clones (unique CDR3 sequence) per cluster", ylab = "BCR clones", xlab = "Sample Cluster",
          color = "cluster", palette = "jco",
          add = "jitter")
dev.off()

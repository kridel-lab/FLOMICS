...
#' @export
FDRcal=function(score.values, meth.tumor, meth.tumor.status,
  transcriptome, no.permutation=100, methylation.event=c('sil','enh'), seed=100){
  ...
  # FDR cutoff
      ...
      cutoff=which((FDR.res$observed.scores>1)&(FDR.res$FDR<=0.1)&(no.false<0.5))
      if(length(cutoff)>0){

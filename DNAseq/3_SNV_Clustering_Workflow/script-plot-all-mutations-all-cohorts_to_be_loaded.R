...
heatmap_mutation_extended <- function(df, genes, group_var, q=0.05, prob_var=NULL,
  ...
  if (y_order == 'dendrogram') {
    ...
    gene_order <- sig_long %>%
      ...
      dplyr::pull(gene)
    gene_order <- unique(gene_order) # Added this to remove duplicates in the case a gene has identical sigs, will keep only the highest sig value position
  } else {
  ...
  pat_order_path = paste0(date, "_", i, "_pat_order_", criteria, ".txt")
  write.table(patient_order, file=pat_order_path, quote=F, row.names=F, sep=";")
  
  df_path = paste0(date, "_", i, "_df_long_", criteria, ".txt")
  write.table(df_long, file=df_path, quote=F, row.names=F, sep=";")
  
  sig_long_out <- sig_long %>% dplyr::mutate(val = -log10(adj_p))
  sig_long_out_path = paste0(date, "_", i, "_sig_long_out_", criteria, ".txt")
  write.table(sig_long_out, file=sig_long_out_path, quote=F, row.names=F, sep=";")

...
muts_all_t1_t2_inner <- muts_all_t1_t2_inner %>%
  ...
  select(1, 2, 4, 3) %>%
  mutate(T1.AIC = str_replace(T1.AIC, "C1", "CS")) %>%
  mutate(T1.AIC = str_replace(T1.AIC, "C2", "TT")) %>%
  mutate(T1.AIC = str_replace(T1.AIC, "C3", "GM")) %>%
  mutate(T1.AIC = str_replace(T1.AIC, "C4", "Q")) %>%
  mutate(T1.AIC = str_replace(T1.AIC, "C5", "AR")) %>%
  mutate(T1.AIC = str_replace(T2.AIC, "C1", "CS")) %>%
  mutate(T1.AIC = str_replace(T2.AIC, "C2", "TT")) %>%
  mutate(T1.AIC = str_replace(T2.AIC, "C3", "GM")) %>%
  mutate(T1.AIC = str_replace(T2.AIC, "C4", "Q")) %>%
  mutate(T1.AIC = str_replace(T2.AIC, "C5", "AR"))

muts_all_t1_t2_inner$T1.AIC <- factor(muts_all_t1_t2_inner$T1.AIC, levels = c("CS", "TT", "GM", "Q", "AR"))
muts_all_t1_t2_inner$T2.AIC <- factor(muts_all_t1_t2_inner$T2.AIC, levels = c("CS", "TT", "GM", "Q", "AR"))
...
clinical_data <- read.csv("clinical_data.csv") %>%
  dplyr::select(LY_FL_ID, SAMPLE_ID, TTT)

#learning the median of TTT (Time To Transformation)
clinical_data <- clinical_data %>%
  dplyr::filter(LY_FL_ID %in% muts_all_T1_T2_inner$LY_FL_ID) %>%
  filter(!is.na(TTT))
clinical_data$TTT = lapply(clinical_data$TTT, as.numeric)
medianTTT <- median(unlist(clinical_data$TTT))
mean(unlist(clinical_data$TTT))

#--
# Spliting data at the median
#--

clinical_data_low <- clinical_data %>%
  dplyr::filter(TTT < medianTTT)
clinical_data_high <- clinical_data %>%
  dplyr::filter(TTT >= medianTTT)

muts_all_t1_t2_inner_low <- muts_all_t1_t2_inner %>%
  dplyr::filter(LY_FL_ID %in% clinical_data_low$LY_FL_ID) # 57 samples
muts_all_t1_t2_inner_high <- muts_all_t1_t2_inner %>%
  dplyr::filter(LY_FL_ID %in% clinical_data_high$LY_FL_ID) # 59 samples

#--
# Graphing alluvial plot
#--

t1_t2_alluvial_low <- muts_all_t1_t2_inner_low %>%
  ...
  ggtitle("Cluster Assignment \n(Time to Transformation < median)")

ggsave(file = paste0(date, "_t1_t2_alluvial_low.png"),
 width = 25, height = 25, units = "cm")

t1_t2_alluvial_high <- muts_all_t1_t2_inner_high %>%
  group_by(T1.AIC, T2.AIC) %>%
  dplyr::summarize(count = n()) %>%
  ggplot(aes(axis1 = T1.AIC, axis2 = T2.AIC, y = count)) +
  scale_x_discrete(limits = c("FL", "tFL"), expand = c(.2, .05)) +
...
ggsave(file = paste0(date, "_t1_t2_alluvial_high.png"),
 width = 25, height = 25, units = "cm")

#--
# Graphing survival plot
#--
